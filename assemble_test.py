...
def main():
    ...
    with tf.device('/device:GPU:1'):
        ...
        for i in range(data_size//batch_size):
            ...
            batch_assemble, batch_accuracy = sess.run([CNN.assemble, CNN.accuracy],
                                                      feed_dict={CNN.X: batch_x, CNN.Y: batch_y})
            assemble_labels = []
            for j in range(batch_size):
                assemble_labels.append(stats.mode(batch_assemble[4*j:4*(j+1)])[0][0])
            assemble_labels = np.asarray(assemble_labels, np.int32)
            # pdb.set_trace()
            nums_correct = np.equal(assemble_labels, cifar.Y[i*batch_size:(i+1)*batch_size])
            batch_assemble_ac = np.mean(nums_correct)
            print("test_step: ", i+1, " test_loss: ", batch_assemble_ac, " test_accuracy: ", batch_accuracy)
            total_assemble_ac.append(batch_assemble_ac)
            total_accuracy.append(batch_accuracy)
        print("Finally, final_assemble_ac:", np.mean(total_assemble_ac), " final_test_accuracy:", np.mean(total_accuracy))
