...
class DenseNet(object):
	def __init__(self, height=28, width=28, depth=1, nums_classes=10, trainable=True, lr=0.001, theta=0.5, prob=0.2):
		self.height = height
		self.width = width
		self.depth = depth
	...
	def build_network(self, activation=tf.nn.relu,
		...
		net = self.X
		# net = self.bn_activation_conv(net, depth=64, size=7, stride=2, padding='same',
									  # is_training=self.is_training, bn=True, bn_scale=True, prob=self.prob)
		net = self.conv_bn_relu(net, depth=64, size=7, stride=7, padding='same', 
								activation=tf.nn.relu, is_training=self.is_training, bn=True, bn_scale=True, prob=self.prob)
		net = tf.layers.max_pooling2d(inputs=net, pool_size=3, strides=2, padding='same')
		# Dense Block1, k = 32, l = 6, input_size = 8
		tmp = net
		for i in range(6):
			net = self.dense_block(input_tensor=tmp, depth=32, is_training=self.is_training, prob=self.prob)
		...
		net = tf.layers.average_pooling2d(inputs=tmp, pool_size=2, strides=2, padding='same')
	...
	def build_optimizer(self):
		...
		with tf.control_dependencies(update_ops):
			rate = tf.train.exponential_decay(learning_rate=self.lr, global_step=self.global_step,
											  decay_steps=400, decay_rate=0.96, staircase=True)
